...
/* 
Given two strings, version1, and version2, both representing
version numbers, if version1 > version2 return 1;

if version1 < version2 return -1; otherwise return 0.

Helpful methods:
  - .split(characterToSplitOn)
    - returns an array of items: "a-b-c".split("-")
      returns ["a", "b", "c"]
  - .parseInt
    - given a string, converts it to and returns int or
...
function compareVersionNumbers(v1, v2) {
  ...
  while (i < v1Arr.length || i < v2Arr.length) {
    const x = parseInt(v1Arr[i]) || 0;
    const y = parseInt(v2Arr[i]) || 0;

    console.log(`i: ${i} x: ${x} vs. y: ${y}`);
